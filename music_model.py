...
class MainUi(QtWidgets.QMainWindow, QDialog):
    ...
    def init_ui(self):
        ...
        self.operator_button_1.setText("歌曲信息")
        ...
        self.operator_button_2.setText("歌曲歌词")
        ...
        self.operator_button_4.setText("收藏歌曲")
        ...
        self.operator_button_6.setText("歌手占比")
        ...
        self.right_operator_layout.addWidget(self.operator_button_1, 1, 0)
        self.right_operator_layout.addWidget(self.operator_button_2, 2, 0)
    ...
    def export_songs_details(self):
        ...
        #生成数据库
        db = pymysql.connect(host='localhost', user='root', password='123456', db='pyhomework', charset='utf8')
        cur = db.cursor()
        ...
        for name in self.names:
            ...
            singer=self.keyword
            url=result+self.tags[row-1]['href']
            add=self.musics[row-1]
            sql = "insert into song_details(song_name,singer,song_url,song_address) VALUES ('%s', '%s', '%s','%s')" % (songname, singer,url,add)
            cur.execute(sql)
            db.commit()
    ...
    def pop_songs(self):
        ...
        for url in urls:
            ...
            songList = jsonData["detail"]["data"]["data"]["song"]
            name = jsonData["detail"]["data"]["data"]["titleShare"]
            for song in songList:
                wb.cell(row=count, column=1, value=count - 1)
                wb.cell(row=count, column=2, value=song["title"])
                wb.cell(row=count, column=3, value=song["singerName"])

                title = song["title"]
                singer = song["singerName"]
                c = count - 1
                sql = "insert into pop_songs(list,song_name,singer) VALUES ('%d', '%s', '%s')" % (c, title, singer)
                cur.execute(sql)
                db.commit()
                count += 1
        ws.save('popsongs.xlsx')
